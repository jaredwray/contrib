...
const UpdateInfluencerProfileMutation = gql`
  mutation UpdateInfluencerProfile(
    ...
    updateMyInfluencerProfile(input: {
      name: $name,
      sport: $sport,
      team: $team,
...
const UpdateInfluencerProfileAvatarMutation = gql`
  mutation UpdateInfluencerProfileAvatar(
    $image: Upload!
  ) {
...
export default function Profile() {
  ...
  const { loading: accountDataLoading, data: myAccountsData, error: accountDataLoadingError } = useQuery<{ myAccount: UserAccount }>(MyAccountQuery);
  ...
  const onSubmit = useCallback(
    ({ name, sport, team, profileDescription }: { name: string, sport: string, team: string, profileDescription: string }) => {
  ...
  useEffect(() => {
    successUpdateMessage && setTimeout(() => {
      setSuccessUpdateMessage("");
    }, HIDE_ALERT_TIMEOUT_MS);
  }, [successUpdateMessage]);

  useEffect(() => {
    updateError && setTimeout(() => {
      setUpdateError("");
    }, HIDE_ALERT_TIMEOUT_MS);
  }, [updateError]);
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Container>
            ...
            <Row className="pt-3 pt-md-0">
              <Col md="6">
                <div className="profile-page-avatar w-100">
                  <input type="file" className="d-none" accept="image/*" ref={ref => (fileInput = ref)} onChange={onFileSelected}/>
                  <Image id="profileAvatar" src={influencerProfile?.avatarUrl} roundedCircle onClick={selectFile}/>
                  <Button className="picture-upload-btn text-label text-all-cups" onClick={selectFile}>change photo</Button>
                </div>
              </Col>
              <Col md="6">
                  <Form.Group>
                    <Form.Label>Name</Form.Label>
                    <Form.Control placeholder="Enter your name" className={formErrors.name && "is-invalid"} name="name" ref={register({required: "required"})} />
                    <ErrorMessage className="invalid-feedback" name="name" as="div" errors={formErrors} />
                  </Form.Group>

                  <Form.Group>
                    <Form.Label>Sport</Form.Label>
                    <Form.Control placeholder="Enter your sport" className={formErrors.sport && "is-invalid"} name="sport" ref={register({required: "required"})} />
                    <ErrorMessage className="invalid-feedback" name="sport" as="div" errors={formErrors} />
                  </Form.Group>

                  <Form.Group>
                    <Form.Label>Team</Form.Label>
                    <Form.Control placeholder="Enter your team name" className={formErrors.team && "is-invalid"} name="team" ref={register({required: "required"})} />
                  ...
                  <Form.Group>
                    ...
                    <Form.Control placeholder="Enter description" as="textarea" rows={5} className={formErrors.profileDescription && "is-invalid"} name="profileDescription" ref={register({required: "required"})} />
            ...
            {!stepByStep &&
              <Row>
                <Col>
                  <Button type="submit" className="btn-ochre float-right">Submit</Button>
                </Col>
              </Row>
            }
          </Container>
          ...
          {stepByStep &&
            <Container fluid className="steps-navigation-container">
              <Row className="pl-4 pr-4">
                ...
                <Col xs="6" className="steps-navigation-items">
                  <Button type="submit" className="btn-with-arrows steps-next-btn float-right">Next</Button>
                </Col>
              </Row>
            </Container>
          }
