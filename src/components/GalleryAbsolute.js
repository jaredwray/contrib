        ...
        <React.Fragment>
            <Row className={props.rowClasses}>
                {data.map((item, index) =>
                    ...
                    >
                        <img src={item.url} className="img-fluid img-gallery" onClick={() => onClick(index)} />
                    </Col>
                )}
            </Row>
            {lightBoxOpen && (
                <Lightbox
                    ...
                    nextSrc={`${data[(activeImage + 1) % data.length].url}`}
                    prevSrc={`${data[(activeImage + data.length - 1) % data.length].url}`}
