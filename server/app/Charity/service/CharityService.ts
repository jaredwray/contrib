...
export class CharityService {
  ...
  async createStripeAccountForCharity(charity: Charity): Promise<Charity> {
    ...
    model.profileStatus = CharityProfileStatus.PENDING_ONBOARDING;
  ...
  async updateCharityByStripeAccount(account: any): Promise<void> {
    ...
    await this.updateCharityStatus(
      charity,
      null,
      account.details_submitted ? StripeCharityStatus.STRIPE_ACTIVE : StripeCharityStatus.STRIPE_INACTIVE,
      session,
    );
  }

  async maybeUpdateStripeLink(charity: Charity): Promise<Charity> {
    if (charity.profileStatus === CharityProfileStatus.PENDING_INVITE) {
      throw new Error('charity can not exist');
    }

    if (charity.profileStatus !== CharityProfileStatus.PENDING_ONBOARDING) {
      return charity;
    }
  ...
  async updateCharityProfileById(id: string, input: UpdateCharityProfileInput): Promise<Charity> {
    ...
    this.maybeUpdateGeneralCharityStatus(charity);

    await charity.save();
    return CharityService.makeCharity(charity);
  }

  private maybeUpdateGeneralCharityStatus(charity: ICharityModel): void {
    if (
      charity.stripeStatus === StripeCharityStatus.STRIPE_ACTIVE &&
      charity.profileStatus === CharityProfileStatus.COMPLETED
    ) {
  ...
  async assignUserToCharity(id: ObjectId, userAccountId: string, session: ClientSession): Promise<Charity> {
    ...
    if (charity.profileStatus !== CharityProfileStatus.PENDING_INVITE) {
      throw new Error(`cannot assign user to charity: charity ${id} status is ${charity.stripeStatus} `);
    }
  ...
  async updateCharityStatus(
    ...
    this.maybeUpdateGeneralCharityStatus(model);
    await model.save();
    return CharityService.makeCharity(model);
  }
